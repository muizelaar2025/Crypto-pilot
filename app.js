// app.js — Crypto Pilot PWA compleet met simulaties
...
function showTab(name){ document.querySelectorAll(".tab").forEach(t=>t.classList.remove("active")); el("tab-"+name).classList.add("active"); }
...
function renderTop10(list){
  ...
  list.forEach((c,i)=>{
    ...
    p.innerHTML=`<span class="pill">${i+1}</span> <strong>${c.name} (${c.symbol.toUpperCase()})</strong> — prijs: ${c.current_price?.toLocaleString(undefined,{maximumFractionDigits:8})} • 7d: ${(c.price_change_percentage_7d_in_currency||0).toFixed(2)}% • score: ${c.opportunity_score.toFixed(2)}
      <button data-id="${c.id}" class="watch-btn">Volgen</button>
      <button data-id="${c.id}" class="chart-btn">Grafiek</button>`;
    container.appendChild(p);
  });
  container.querySelectorAll(".watch-btn").forEach(btn=>btn.addEventListener("click",()=>{ el("hold-coin").value=btn.dataset.id; el("hold-qty").focus(); }));
  container.querySelectorAll(".chart-btn").forEach(btn=>btn.addEventListener("click",()=>{ populateChartDropdown(); el("chart-coin").value=btn.dataset.id; showTab('charts'); }));
...
async function getPrice(coinId,vs="eur"){ try{ const r=await fetch(`${CG_API}/simple/price?ids=${coinId}&vs_currencies=${vs}`); if(!r.ok) throw new Error("HTTP "+r.status); const j=await r.json(); return j[coinId]?j[coinId][vs]:null;}catch(e){console.error("getPrice err",e);return null;} }
function adviceForHolding(h,currentPrice,isInTop10){ const gross=(currentPrice-h.buyPrice)*h.qty; const net=gross-TRANSACTION_FEE; if(net>0) return {advice:"Behouden",reason:`Winst (€${net.toFixed(2)})`}; return {advice:isInTop10?"Behouden":"Verkopen",reason:`Verlies €${net.toFixed(2})`}; }
async function renderHoldings(){
  ...
  for(const h of holdings){
    ...
    tr.innerHTML=`<td>${h.id}</td><td>${h.qty}</td><td>${h.buyPrice.toFixed(4)}</td><td>${price!==null?price.toLocaleString(undefined,{maximumFractionDigits:8}):"?"}</td><td>${net!==null?"€"+net.toFixed(2):"?"}</td><td class="${ad.advice==='Behouden'?'advice-hold':'advice-sell'}">${ad.advice}<br><small>${ad.reason}</small></td><td><button data-id="${h.id}" class="del">Verwijder</button></td>`;
    tbody.appendChild(tr);
  }
  tbody.querySelectorAll(".del").forEach(b=>b.addEventListener("click",()=>{ holdings=holdings.filter(x=>x.id!==b.dataset.id); renderHoldings(); }));
}
...
async function renderAdviceLog(){
  ...
  for(let i=0;i<logs.length;i++){
    ...
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${l.time}</td><td>${l.coin}</td><td>${l.advice}</td><td>${l.startPrice!==undefined?"€"+Number(l.startPrice).toFixed(4):"?"}</td><td>${price!==null?"€"+price.toFixed(4):"?"}</td><td>${l.result}</td><td><button data-i="${i}" class="unfollow">Niet meer volgen</button></td>`;
    tbody.appendChild(tr);
  }
  tbody.querySelectorAll(".unfollow").forEach(b=>b.addEventListener("click",()=>{ const i=Number(b.dataset.i); const logs2=loadAdviceLogs(); logs2.splice(i,1); saveAdviceLogs(logs2); renderAdviceLog(); }));
}

// ---------------- Charts ----------------
async function loadChartData(coin,days,vs="eur"){ try{ const r=await fetch(`${CG_API}/coins/${coin}/market_chart?vs_currency=${vs}&days=${days}`); if(!r.ok) throw new Error("HTTP "+r.status); const data=await r.json(); return data.prices?data.prices.map(p=>({x:new Date(p[0]),y:p[1]})) : [];}catch(e){console.error(e);return [];} }
el("loadChart").addEventListener("click",async()=>{
  ...
  chart=new Chart(ctx,{type:"line",data:{datasets:[{label:`${coin} prijs`,data:prices,borderColor:"blue",fill:false,pointRadius:0}]},options:{scales:{x:{type:"time",time:{unit:days>90?"month":"day"}},y:{beginAtZero:false}}}});
});
function populateChartDropdown(){
  const sel=el("chart-coin");
  const prev=sel.value;
  sel.innerHTML="";
  if(currentTop10&&currentTop10.length>0){ currentTop10.forEach(c=>{ const opt=document.createElement("option"); opt.value=c.id; opt.textContent=`${c.name} (${c.id})`; sel.appendChild(opt); }); if(prev) sel.value=prev; } else { const opt=document.createElement("option"); opt.value=""; opt.textContent="Laad eerst Top10 (Refresh selection)"; sel.appendChild(opt); }
}
...
el("stopMonitor").addEventListener("click",()=>{ if(monitorHandle){ clearInterval(monitorHandle); monitorHandle=null; log("Monitor gestopt"); } if(window._adviceChecker){ clearInterval(window._adviceChecker); window._adviceChecker=null; } });
